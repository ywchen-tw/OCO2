...
def cal_ocean_brdf(
    ...
    with h5py.File(f'{sat0.fdir_pre_data}/pre-data.h5', 'r') as f_pre_data:
        ...
        u_10m  = f_pre_data['oco/met/u_10m'][...]
        v_10m  = f_pre_data['oco/met/v_10m'][...]
        oco_lon = f_pre_data['oco/met/lon'][...]
        oco_lat = f_pre_data['oco/met/lat'][...]
    points = np.column_stack((oco_lon, oco_lat))
    ws_10m = np.sqrt(u_10m**2 + v_10m**2)

    popt_diff, pcov = curve_fit(func, oco_lat, ws_10m)
    u10 = griddata(points, func(oco_lat, *popt_diff), 
                              (data['lon_2d']['data'], data['lat_2d']['data']), method='nearest')
...
def oco_ocean_brdf(
    ...
    with h5py.File(sat0.fnames['oco_std'][0], 'r') as f:
        lat    = f['RetrievalGeometry/retrieval_latitude'][...]
        lon    = f['RetrievalGeometry/retrieval_longitude'][...]
        lambertian_albedo_o2a = f['AlbedoResults/albedo_o2_fph'][...]
        lambertian_slope_o2a = f['AlbedoResults/albedo_slope_o2'][...]
        lambertian_albedo_wco2 = f['AlbedoResults/albedo_weak_co2_fph'][...]
        lambertian_slope_wco2 = f['AlbedoResults/albedo_slope_weak_co2'][...]
        lambertian_albedo_sco2 = f['AlbedoResults/albedo_strong_co2_fph'][...]
        lambertian_slope_sco2 = f['AlbedoResults/albedo_slope_strong_co2'][...]
        logic = (lon>=sat0.extent[0]) & (lon<=sat0.extent[1]) & \
                (lat>=sat0.extent[2]) & (lat<=sat0.extent[3])

    if band=='o2a':
        refrac_r = np.ones_like(lambertian_albedo_o2a[logic])*1.331
        diffuse_alb = lambertian_albedo_o2a[logic]
        slope = lambertian_slope_o2a[logic]
    elif band=='wco2':
        refrac_r = np.ones_like(lambertian_albedo_wco2[logic])*1.318
        diffuse_alb = lambertian_albedo_wco2[logic]
        slope = lambertian_slope_wco2[logic]
    elif band=='sco2':
        refrac_r = np.ones_like(lambertian_albedo_sco2[logic])*1.303
        diffuse_alb = lambertian_albedo_sco2[logic]
        slope = lambertian_slope_sco2[logic]
    ...
    lat_sort = np.argsort(lat[logic])
    # print('lat_sort:', lat_sort)
    # plt.clf()
    # plt.plot(diffuse_alb[lat_sort], lat[lat_sort])
    # plt.show()
    
    # plt.scatter(lon[logic], lat[logic], c=diffuse_alb, s=5)
    # plt.colorbar()
    # plt.show()
    ...
    points = np.column_stack((lon[logic], lat[logic]))

    popt_diff, pcov = curve_fit(func, lat[logic], diffuse_alb)
    diffuse_alb_2d = griddata(points, func(lat[logic], *popt_diff), 
                              (data['lon_2d']['data'], data['lat_2d']['data']), method='nearest')
    popt_refrac, pcov = curve_fit(func, lat[logic], refrac_r)
    refrac_r_2d = griddata(points, func(lat[logic], *popt_refrac), 
                              (data['lon_2d']['data'], data['lat_2d']['data']), method='nearest')
    popt_slope, pcov = curve_fit(func, lat[logic], slope)
    slope_2d = griddata(points, func(lat[logic], *popt_slope), 
                              (data['lon_2d']['data'], data['lat_2d']['data']), method='nearest')

    print(slope_2d.shape)
    print(diffuse_alb_2d.shape)
    print(data['lon_2d']['data'].shape)
    # plt.contourf(data['lon_2d']['data'], data['lat_2d']['data'], diffuse_alb_2d)
    # plt.colorbar()
    # plt.show()


    params = {
          'diffuse_alb': diffuse_alb_2d,
         'diffuse_frac': np.zeros_like(diffuse_alb_2d),
             'refrac_r': refrac_r_2d,
             'refrac_i': np.zeros_like(diffuse_alb_2d),
                'slope': slope_2d,
            }
