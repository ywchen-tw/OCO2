...
#---------------------------------------------------------------------------
from util.abs.read_atm import read_oco_zpt
from util.abs.rdabs_gas import rdabs_species
from util.abs.find_bound import find_boundary # get wavenumber indices.initialize()import abs/get_index  # find levels in absco files that are closest to the atmosphere
from util.abs.get_index import get_PT_index  # find levels in absco files that are closest to the atmosphere.initialize()import abs/rdabscoo2.pro   # read absorption coefficients O2
from util.abs.rdabsco_gas import rdabsco_species   # read absorption coefficients O2.initialize()import abs/rdabscoco2.pro  # read absorption coefficients CO2
from util.abs.calc_ext import calc_ext   # calculates extinction profiles & layer transmittance from oco_util.absorption coefficients & density profiles (CO2, O2)
...
from util.abs.solar import solar   # read solar file
from util.abs.oco_convolve import oco_conv
from util.abs.oco_abs_g_mode import oco_wv_select
from util.oco_util import timing
from util.oco_cfg import grab_cfg
...
def oco_abs(cfg, sat, zpt_file, 
    ...
    # ******************************************************************************************************
    # Extract information from oco_util.absCOF files (native resolution).
    ...
    if not os.path.isfile(savpkl) or reextract==True:
        ...
        refl = [alb_o2a, alb_wco2, alb_sco2][iband] #surface reflectance
    ...
    # End: Extract information from oco_util.absCOF file (native resolution)
