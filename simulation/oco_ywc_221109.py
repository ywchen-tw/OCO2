...
def pre_cld(sat, tag, cth=None, cot_source='2stream', ref_threshold=0.1, scale_factor=1.0, solver='3D'):
    ...
    _, _, mcd_bsa_2d = grid_by_extent(mcd43.data['lon']['data'], mcd43.data['lat']['data'], mcd43.data['bsa']['data'][0, :], extent=sat.extent)
    _, _, mcd_wsa_2d = grid_by_extent(mcd43.data['lon']['data'], mcd43.data['lat']['data'], mcd43.data['wsa']['data'][0, :], extent=sat.extent)
...
def run_case(band_tag, cfg_info):
    ...
    extent[3] += 0.15
    print(extent)
    ref_threshold = cfg_info['ref_threshold']
