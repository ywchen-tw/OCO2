...
class satellite_download:
    ...
    def run(self, run=True):
        ...
        for filename_tag in filename_tags_43:
            fnames_43 = download_modis_https(self.date, '61/MCD43A3', filename_tag, day_interval=16, fdir_out=self.fdir_out, run=run)
...
def pre_cld(sat, tag, cth=None, ref_threshold=0.1, scale_factor=1.0, solver='3D'):
    ...
    modl2_1km      = modis_l2(fnames=sat.fnames['mod_l2'], extent=sat.extent,
                              vnames=['cloud_top_height_1km', 'cloud_top_temperature_1km', 
                                     'Cloud_Optical_Thickness', 'Cloud_Optical_Thickness_16',
                                     'Cloud_Optical_Thickness_PCL', 'Cloud_Optical_Thickness_1621',
    ...
    logic_cth_1km  = np.logical_and(modl2_1km.data['cloud_top_height_1km']['data']>0.0, modl2_1km.data['cloud_top_height_1km']['data']<14.0)        

    cot0       = modl2_1km.data['cot']['data']
    #"""
    ...
    rgb_grid_b    = interpolate.griddata(points_rgb, mod_b.flatten(), (lon_2d.flatten(), lat_2d.flatten()), method='nearest').reshape(lon_2d.shape)
    ...
    if cth is None:
        ...
        """
        rbf = interpolate.Rbf(lon_1km[logic_cth_1km], lat_1km[logic_cth_1km], cth_1km[logic_cth_1km], function='gaussian')
        cth = rbf(lon, lat)
        logic = np.isnan(cth)
        cth[logic] = 0.0
        """
        ...
        cer_2d_l2 = np.zeros_like(lon_2d)
        cer_2d_l2[indices_x, indices_y] = cer
        cer_2d_l2[cer_2d_l2<=1] = 1
...
def pre_sfc(sat, tag, version='10r', scale=True, replace=True):
    ...
    print(sat.fnames['mcd_43'])
    print('sat extent', sat.extent)
    print(mod.data['lon']['data'].shape)
    points = np.transpose(np.vstack((mod.data['lon']['data'], mod.data['lat']['data'])))
    if tag.lower() == 'o2a':
        lon_2d, lat_2d, mod_sfc_alb_2d = grid_modis_by_extent(mod.data['lon']['data'], mod.data['lat']['data'], mod.data['bsa']['data'][1, :], extent=sat.extent)
    elif tag.lower() == 'wco2':
        lon_2d, lat_2d, mod_sfc_alb_2d = grid_modis_by_extent(mod.data['lon']['data'], mod.data['lat']['data'], mod.data['bsa']['data'][5, :], extent=sat.extent)
    elif tag.lower() == 'sco2':
        lon_2d, lat_2d, mod_sfc_alb_2d = grid_modis_by_extent(mod.data['lon']['data'], mod.data['lat']['data'], mod.data['bsa']['data'][6, :], extent=sat.extent)
...
def susan_case(band_tag, cfg_info):
    ...
    for wavelength in wvls:
        for solver in ['IPA', '3D']:
            cal_mca_rad_oco2(date, band_tag, sat0, wavelength, fname_idl=fname_idl, cth=None, scale_factor=1.0, sfc_scale=True, sfc_replace=True, fdir=fdir_tmp, solver=solver, overwrite=True, photons=1e8, ref_threshold=0.04)
    # ===============================================================
