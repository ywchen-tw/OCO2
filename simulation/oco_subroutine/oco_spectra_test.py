...
def oco_abs(cfg, zpt_file, iband=0, nx=None, Trn_min=0, pathout=None, reextract=True, plot=False):
    ...
    if not os.path.isfile(savpkl) or reextract==True:
        ...
        intf, pintf, pprf, tprf, o2den, co2den, h2oden, h2o_vmr, dzf, lay = read_oco_zpt(zpt_file=zpt_file)
    ...
    if (save == 1 | plot == 1):
        ...
        mx = 1.            # maximum transmittance (T) within spectral sub-range
        mn = np.max([Trn_min, np.min(trnsx)])    # minimum transmittance with spectral sub-range
        ...
        if plot == 1 :
            ...
            ax.plot([wli0,nfc],[trnsx[wli0],trnsx[wli0]],linestyle='dashed',color='orange')
            fig.savefig(f'{pathout}/band{iband}_2-test.png', dpi=150, bbox_inches='tight')
        ...
        if plot == 1 :
            ...
            #
            ax.plot([wr[0],wr[0]],[0,1], color='orange', linestyle='--', linewidth=3)
            ax.plot([wr[1],wr[1]],[0,1], color='orange', linestyle='--', linewidth=3)
            #!P.multi=0
        ...
        if plot == 1 :
            fig.savefig(f'{pathout}/band{iband}_1-test.png', dpi=150, bbox_inches='tight')
            #plt.show()
