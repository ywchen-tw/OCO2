...
def pre_cld(sat, cth=None, scale_factor=1.0, solver='3D'):
    ...
    yy_2stream = cal_r_twostream(xx_2stream, a=a0, mu=mu0) # to get reflectance and COT relation

    threshold  = a0 * 2.0 # needs to be improved
    indices    = np.where(ref_2d>threshold) # identify which pixels are cloud
    indices_x  = indices[0]
    indices_y  = indices[1]
...
def cal_mca_rad_oco2(date, tag, sat, wavelength, fname_idl=None, cth=None, photons=2e10, scale_factor=1.0, sfc_scale=True, sfc_replace=True, fdir='tmp-data', solver='3D', overwrite=True):
    ...
    if cth is None:
        ...
        cth0 = cth
    print('theckness data:-------')
    print(np.unique(atm0.lay['thickness']['data'])[0])
    print((atm0.lay['thickness']['data']))
