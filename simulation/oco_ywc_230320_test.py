...
def run_case(band_tag, cfg_info, sfc_alb=None, sza=None):
    ...
    if sfc_alb != None:
        fdir_tmp = os.path.abspath('tmp-data/%s_alb_%.2f_saz_%.1f/%s' % (name_tag, sfc_alb, sza, band_tag))
    else:
        fdir_tmp = os.path.abspath('tmp-data/%s/%s' % (name_tag, band_tag))
    print(fdir_tmp)
    ...
    if 1:#not os.path.isfile(fname_abs):
        oco_abs(cfg, zpt_file=zpt_file, iband=iband_dict[band_tag], nx=nx, Trn_min=Trn_min, pathout=fdir_data, reextract=True, plot=True)
    f = h5py.File(fname_abs, 'r')
    wvls = f['lamx'][...]*1000.0
