...
def pre_cld(sat, cth=None, scale_factor=1.0, solver='3D'):
    ...
    threshold  = a0 * 1.0 # needs to be improved
    ...
    for i in range(indices_x.size):
        if 0<=indices_x_new[i]<Nx and 0<=indices_y_new[i]<Ny:
            cot_2d_l1b[indices_x_new[i], indices_y_new[i]] = xx_2stream[np.argmin(np.abs(yy_2stream-ref_860_2d[indices_x[i], indices_y[i]]))]
    ...
    modl1b.data['ref_2d'] = dict(name='Gridded reflectance'             , units='N/A'        , data=ref_860_2d)
...
def susan_case(band_tag, cfg_info):
    ...
    for wavelength in wvls:
        for solver in ['IPA', '3D']:
            cal_mca_rad_oco2(date, band_tag, sat0, wavelength, fname_idl=fname_idl, cth=None, scale_factor=1.0, sfc_scale=True, sfc_replace=True, fdir=fdir_tmp, solver=solver, overwrite=True, photons=4e6)
    # ===============================================================
