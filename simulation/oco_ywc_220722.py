...
def pre_cld(sat, cth=None, scale_factor=1.0, solver='3D'):
    ...
    rgb_grid    = interpolate.griddata(points_rgb, mod_v_over_s.flatten(), (lon_2d.flatten(), lat_2d.flatten()), method='nearest').reshape(lon_2d.shape)
    #indices    = np.where(ref_650_2d*ref_860_2d>threshold)
    indices = np.where(rgb_grid > 4)
