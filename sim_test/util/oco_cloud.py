...
def cdata_cld_modis_only(sat0, fdir_cot, zpt_file, cfg_info, plot=True):
    ...
    print('sza mean:', sza.mean())
    print('saa mean:', saa.mean())
    print('vza mean:', vza.mean())
    print('vaa mean:', vaa.mean())
    # print('sfh mean:', sfh.mean())
    saa = saa[saa!=0]
    sza = sza[sza!=0]
    vza = vza[vza!=0]
    vaa = vaa[vaa!=0]
    ...
    #/----------------------------------------------------------------------------\#
    cld_msk_ipa0_650 = np.zeros_like(ref_2d)
