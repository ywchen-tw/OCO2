...
def cal_mca_rad_1640(sat, zpt_file, wavelength, cfg_info, fdir='tmp-data', solver='3D', case_name_tag='default', overwrite=False):
    wavelength = 1640.0
    """
    ...
    # abs object
    # =================================================================================
    fname_abs = '%s/abs_1640.pk' % fdir
    abs0      = abs_16g(wavelength=wavelength, fname=fname_abs, atm_obj=atm0, overwrite=True)
    # =================================================================================
    ...
    with h5py.File(f'{sat.fdir_pre_data}/pre-data.h5', 'r') as f:
        data['alb_2d'] = dict(data=f['mod/sfc/alb_43_1640'][...], name='Surface albedo (lambertian)', units='N/A')
    ...
    fname_sfc = '%s/sfc_1640.pk' % fdir
    mod09 = sat_tmp(data)
    sfc0      = sfc_sat(sat_obj=mod09, fname=fname_sfc, extent=sat.extent, verbose=True, overwrite=overwrite)
    sfc_2d    = mca_sfc_2d(atm_obj=atm0, sfc_obj=sfc0, fname='%s/mca_sfc_2d_1640.bin' % fdir, overwrite=overwrite)
    # =================================================================================
    ...
    with h5py.File(f'{sat.fdir_pre_data}/pre-data.h5', 'r') as f_pre_data:
        ...
        data['rad_2d'] = dict(name='Gridded radiance'                , units='km'         , data=f_pre_data[f'mod/rad/rad_1640'][...])
        if solver.lower() == 'ipa':
    ...
    sca  = mca_sca(pha_obj=pha0, fname='%s/mca_sca_1640.bin' % fdir, overwrite=overwrite)
    # =================================================================================

    # mca_cld object
    # =================================================================================
    atm3d0  = mca_atm_3d(cld_obj=cld0, atm_obj=atm0, pha_obj=pha0, fname='%s/mca_atm_3d_1640.bin' % fdir)
    ...
    with h5py.File(f'{sat.fdir_pre_data}/pre-data.h5', 'r') as f:
        ...
        #"""
        sza_all = f['mod/geo/sza'][...]
        saa_all = f['mod/geo/saa'][...]
        vza_all = f['mod/geo/vza'][...]
        vaa_all = f['mod/geo/vaa'][...]
        saa = saa_all[saa_all!=0].mean()
        sza = sza_all[sza_all!=0].mean()
        vza = vza_all[vza_all!=0].mean()
        vaa = vaa_all[vaa_all!=0].mean()
        #"""
    # =================================================================================
    ...
    # mcarats output
    out0 = mca_out_ng(fname='%s/mca-out-rad-modis-%s_%.4fnm.h5' % (fdir, solver.lower(), wavelength), mca_obj=mca0, abs_obj=abs0, mode='mean', squeeze=True, verbose=True, overwrite=True)
    # =================================================================================

def modis_1640_simulation_plot(sat, cfg_info, case_name_tag='default', fdir='tmp',
                               solver='3D', wvl=1640, plot=False):
    ...
    with h5py.File('data/%s/pre-data.h5' % case_name_tag, 'r') as f:
        ...
        lon_mod = f['lon'][...]
        lat_mod = f['lat'][...]
        rad_mod = f['mod/rad/rad_1640'][...]
        ref_mod = f['mod/rad/ref_1640'][...]
    ...
    with h5py.File(fname, 'r') as f:
        ...
        toa = f['mean/toa'][...]
    # ==================================================================================================
    # plt.scatter(lon_mod, lat_mod, c=rad_rtm_sim/toa, cmap='jet', vmin=0, vmax=1)
    # plt.colorbar()
    # plt.show()

    # plt.scatter(lon_mod, lat_mod, c=rad_rtm_sim/(toa*np.cos(sza/180)*np.pi)*np.pi, cmap='jet', vmin=0, vmax=1)
    # plt.colorbar()
    # plt.show()
    # sys.exit()
    # save data
    ...
    # c1 = ax1.imshow(rad_mod, extent=mod_img_wesn)
    # ax1.set_title('MODIS Measured Radiance')
    # cbar = fig.colorbar(c1, ax=ax1, orientation='vertical', pad=0.05, fraction=0.0665)
    # c2 = ax2.imshow(rad_rtm_sim, extent=mod_img_wesn)
    # ax2.set_title(f'EaR$^3$T Simulated {solver} Radiance')
    ...
    if plot:
        ...
        ax1.pcolormesh(lon_mod, lat_mod, rad_mod, cmap='Greys_r', vmin=0.0, vmax=0.15)
        ax1.set_title('MODIS Measured Radiance')
        ...
        ax3.pcolormesh(lon_mod, lat_mod, rad_rtm_sim, cmap='Greys_r', vmin=0.0, vmax=0.15)
        ...
        plt.savefig(f'{sat.fdir_pre_data}/modis_1640_{case_name_tag}_{solver}.png', bbox_inches='tight')
        ...
        ax1.imshow(mod_img, extent=mod_img_wesn)
        c1 = ax1.pcolormesh(lon_mod, lat_mod, rad_mod, cmap='Greys_r', vmin=0.0, vmax=0.15)
        cbar = fig.colorbar(c1, ax=ax1, orientation='vertical', pad=0.05, fraction=0.0665)
        ...
        c2 = ax2.pcolormesh(lon_mod, lat_mod, rad_rtm_sim, cmap='Greys_r', vmin=0.0, vmax=0.15)
        cbar2 = fig.colorbar(c2, ax=ax2, orientation='vertical', pad=0.05,fraction=0.0665)
        ...
        plt.subplots_adjust(hspace=0.4, wspace=0.4)
        plt.savefig(f'{sat.fdir_pre_data}/modis_1640_{case_name_tag}_{solver}_comparison.png', bbox_inches='tight', dpi=300)
        # ==================================================================================================
        ...
        ax1 = fig.add_axes([0.05, 0.05, 0.21, 0.9])
        ax1.imshow(mod_img, extent=mod_img_wesn)
        c1 = ax1.pcolormesh(lon_mod, lat_mod, ref_mod, cmap='Greys_r', vmin=0.0, vmax=0.5)
        cbar = fig.colorbar(c1, ax=ax1, orientation='vertical', pad=0.05, fraction=0.0665)
        cbar.set_label('MODIS Measured Reflectance', fontsize=label_size-2  )

        ax2 = fig.add_axes([0.37, 0.05, 0.21, 0.9])
        ax2.imshow(mod_img, extent=mod_img_wesn)
        c2 = ax2.pcolormesh(lon_mod, lat_mod, rad_rtm_sim*np.pi/(toa*np.cos(sza/180*np.pi)), cmap='Greys_r', vmin=0.0, vmax=0.5)
        cbar2 = fig.colorbar(c2, ax=ax2, orientation='vertical', pad=0.05,fraction=0.0665)
        cbar2.set_label('Simulated %s Reflectance' %solver, fontsize=label_size-2)
        
        xedges = np.arange(-0.01, 0.61, 0.005)
        yedges = np.arange(-0.01, 0.61, 0.005)
        heatmap, xedges, yedges = np.histogram2d(ref_mod[logic], (rad_rtm_sim*np.pi/(toa*np.cos(sza/180*np.pi)))[logic], bins=(xedges, yedges))
        YY, XX = np.meshgrid((yedges[:-1]+yedges[1:])/2.0, (xedges[:-1]+xedges[1:])/2.0)
        ...
        ax3.set_xlabel(r'MODIS Measured Reflctance', fontsize=label_size)
        ax3.set_ylabel(r'Simulated %s Reflctance' %solver, fontsize=label_size)
        ...
        plt.subplots_adjust(hspace=0.4, wspace=0.4)
        plt.savefig(f'{sat.fdir_pre_data}/modis_1640_{case_name_tag}_{solver}_comparison_ref.png', bbox_inches='tight', dpi=300)
        # ==================================================================================================
        ...
        plt.subplots_adjust(hspace=0.4, wspace=0.4)
        plt.savefig(f'{sat.fdir_pre_data}/modis_1640_cloud_information.png', bbox_inches='tight')
        plt.clf()
        fig = plt.figure(figsize=(15, 5))
        ax1 = fig.add_axes([0.05, 0.05, 0.25, 0.9])
        ax2 = fig.add_axes([0.40, 0.05, 0.25, 0.9])
