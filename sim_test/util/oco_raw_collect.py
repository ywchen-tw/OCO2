...
def cdata_sat_raw(sat0, dx, dy, overwrite=False, plot=True):
    ...
    if os.path.isfile(f'{sat0.fdir_pre_data}/pre-data.h5') and not overwrite:
        ...
        # cloud properties
        #/--------------------------------------------------------------\#
        modl2 = modis_l2(fnames=sat0.fnames['mod_l2'], extent=sat0.extent, vnames=['cloud_top_height_1km',  'Cloud_Effective_Radius', 'Cloud_Optical_Thickness'])
        ...
        cer0 = modl2.data['Cloud_Effective_Radius']['data']
        cer0[cer0<=1.0] = np.nan
        
        cot0 = modl2.data['Cloud_Optical_Thickness']['data']
        cot0[cot0<=0.0] = np.nan
