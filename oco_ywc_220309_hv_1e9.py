...
def susan_case(band_tag, cfg_info):
    ...
    for wavelength in wvls:
        for solver in ['IPA', '3D']:
            cal_mca_rad_oco2(date, band_tag, sat0, wavelength, fname_idl=fname_idl, cth=None, scale_factor=1.0, sfc_scale=True, sfc_replace=True, fdir=fdir_tmp, solver=solver, overwrite=True, photons=1e9)
    # ===============================================================
...
if __name__ == '__main__':
    ...
    for i in range(len(sb)):
        ...
        if not os.path.exists(cfg):
            os.system('cp cfg/20150622_template.csv {}'.format(cfg))
            save_subdomain_info(cfg, sb[i])
        run_simulation(cfg)
