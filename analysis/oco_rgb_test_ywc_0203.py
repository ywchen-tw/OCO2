...
class satellite_download:
    ...
    def run(self, run=True):
        ...
        """
        for filename_tag in ['GA']:    
            fnames_09_GA = download_modis_https(self.date, '6/MYD09GA', filename_tag, day_interval=1, fdir_out=self.fdir_out, run=run)
            self.fnames['mod_09_GA'] += fnames_09_GA
        """
...
def pre_cld_oco(sat, mode='rgb', cth=None, scale_factor=1.0, solver='3D'):
...
def run_er3t_case_01(band_tag, cfg_info):
...
def run_simulation(cfg):
    ...
    if 1:#not check_h5_info(cfg, 'o2'): 
        if 1:
            ...
            o2_h5 = run_er3t_case_01('o2a', cfg_info)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Read the csv file as setting info to calculate 3d effect.')
    parser.add_argument('cfg', metavar='cfg', type=str, nargs=1,
                        help='path of the csv setting file')
