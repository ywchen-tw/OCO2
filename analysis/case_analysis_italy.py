...
def main(cfg_name='20151219_north_italy_470cloud_test.csv'):
    ...
    c = ax.contourf(lon_2d,lat_2d, 
                   sfh_2d*1000,
                   cmap='terrain', levels=201, vmin=0, vmax=1000)
    ...
    f.tight_layout()
    f.savefig(f'north_italy_surface_altitude.png', dpi=300)
    ...
    ax.set_ylabel('Latitude ($^\circ$N)', fontsize=label_size)
    f.tight_layout()
    f.savefig(f'north_italy_cloud_distance.png', dpi=300)
    # plt.show()
    ...
    ax.set_ylabel('Latitude ($^\circ$N)', fontsize=label_size)
    f.tight_layout()
    f.savefig(f'north_italy_o2a_conti_{rad_c3d_compare}.png', dpi=300)
    # plt.show()
    ...
    f.tight_layout()
    f.savefig(f'north_italy_o2a_{slope_compare}.png', dpi=300)
    plt.show()
...
def heatmap_xy_3(x, y, ax):
    ...
    for cld_min in [1, 1., 1.5, 1.75]:
        for cld_max in np.arange(10, 18, 0.5):
            ...
            popt, pcov = curve_fit(func, xx, yy, bounds=([-10, 1e-3], [10, 15,]),
                                p0=(0.1, 0.7),
                                maxfev=3000,
                                #sigma=value_std[val_mask], 
                                #absolute_sigma=True,
...
def fitting(cloud_dist, rad_3d, rad_clr, slope, inter, band, plot=False):
    ...
    if plot:
        ...
        fig.savefig(f'north_italy_{band}.png', dpi=150, bbox_inches='tight')
        #plt.show()
    else:
        mask = np.logical_and(cloud_dist > 0, rad_3d>rad_clr)
...
def fitting_3bands(cloud_dist, o1, o2, o3, rad_3d_compare, rad_clr_compare, slope_compare, inter_compare, region_mask):
    ...
    #ax.set_yscale('log')
    # fig.suptitle(f"sfc albedo={alb:.2f}, sza={sza:.1f}$^\circ$")
    fig.savefig(f'north_italy_all_band_{slope_compare.split("_")[-1]}.png', dpi=150, bbox_inches='tight')
    #plt.show()
