...
def main(cfg_name='20181018_central_asia_2.csv'):
    ...
    retrieval_aod_parameterization = h5py.File('full-unperturbed20181018_central_asia_2.h5', 'r')
    ...
    retrieval_pixel = h5py.File('full-unperturbed20181018_central_asia_2_pbp.h5', 'r')
    ...
    print(f"mean df_pixel['diff_xco2']: {np.mean(df_pbp['diff_xco2']):.3f} +/- {np.std(df_pbp['diff_xco2']):.3f}")
    print(f"mean df['diff_xco2']: {np.mean(df['diff_xco2']):.3f} +/- {np.std(df['diff_xco2']):.3f}")

    mask = cld_dist>15
    print(f"mean df_pixel['diff_xco2'] for cld_dist > 5: {np.mean(df_pbp['diff_xco2'][mask]):.3f} +/- {np.std(df_pbp['diff_xco2'][mask]):.3f}")
    print(f"mean df['diff_xco2'] for cld_dist > 15: {np.mean(df['diff_xco2'][mask]):.3f} +/- {np.std(df['diff_xco2'][mask]):.3f}")


    retrieval_para = h5py.File('full-unperturbed20181018_central_asia_2_para.h5', 'r')
    mask = retrieval_para['xco2_retrieved'][...]!=-2
    key_list = ['aod', 'cpu_minutes', 'lat', 'lon', 'psur_MT_file', 'psur_retrieved',
                'rfl1', 'rfl2', 'rfl3', 'snd', 'xco2_L2_file', 'xco2_retrieved', 'xco2_weighted_column']
    df_para = pd.DataFrame({key:retrieval_para[key][...] for key in key_list})
    df_para.replace(-2, np.nan, inplace=True)
    df_para.loc[df_para['xco2_L2_file']<1, 'xco2_L2_file'] = df_para.loc[df_para['xco2_L2_file']<1, 'xco2_L2_file']*1e6
    df_para.drop_duplicates('snd', inplace=True)
    df_para['diff_xco2'] = df_para['xco2_retrieved']-df_para['xco2_L2_file']



    print(f"mean df_para['diff_xco2']: {np.mean(df_para['diff_xco2']):.3f} +/- {np.std(df_para['diff_xco2']):.3f}")
    print(f"mean df['diff_xco2']: {np.mean(df['diff_xco2']):.3f} +/- {np.std(df['diff_xco2']):.3f}")
    print(f"mean df_pixel['diff_xco2']: {np.mean(df_pbp['diff_xco2']):.3f} +/- {np.std(df_pbp['diff_xco2']):.3f}")
    ...
    mask = cld_dist>15
    print(f"mean df['diff_xco2'] for cld_dist > 15: {np.mean(df['diff_xco2'][mask]):.3f} +/- {np.std(df['diff_xco2'][mask]):.3f}")
    print(f"mean df_pbp['diff_xco2'] for cld_dist > 15: {np.mean(df_pbp['diff_xco2'][mask]):.3f} +/- {np.std(df_pbp['diff_xco2'][mask]):.3f}")
    print(f"mean df_para['diff_xco2'] for cld_dist > 15: {np.mean(df_para['diff_xco2'][mask]):.3f} +/- {np.std(df_para['diff_xco2'][mask]):.3f}")
    ...
    delta_XCO2_comparison_para_para_ori_pixel(cld_dist, df, df_pbp, df_para, img_dir=img_dir)
...
def delta_XCO2_comparison_para_para_ori_pixel(cld_dist, df, df_pbp, df_para,
                                     img_dir='.', label_size=16, tick_size=14):
    fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(18, 4.8), sharex=False)
    fig.tight_layout(pad=5.0)
    light_jet = cm.jet#cmap_map(lambda x: x/3 + 0.66, cm.jet)
    ...
    ax2.scatter(cld_dist, df_para['diff_xco2'], color='k', s=15)
    ax2.tick_params(axis='both', labelsize=tick_size)
    ymin2, ymax2 = ax2.get_ylim()
    xmin2, xmax2 = ax2.get_xlim()

    ax3.scatter(cld_dist, df_pbp['diff_xco2'], color='k', s=15)
    ...
    # ax.set_title('title', fontsize=title_size)
    fig.tight_layout(pad=2.0)
    fig.savefig(f'{img_dir}/Delta_XCO2_comparison_cld_para_test_para_ori_pbp.png', dpi=300)
