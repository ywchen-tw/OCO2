...
import platform
from er3t.pre.abs import abs_16g, abs_16g_txt
...
def cal_mca_rad_2130(sat, zpt_file, wavelength, cfg_info, fdir='tmp-data', solver='3D', case_name_tag='default', overwrite=False):
    wavelength = 2130.0
    """
    ...
    data = {}
    with h5py.File(f'{sat.fdir_pre_data}/pre-data.h5', 'r') as f:
        data['alb_2d'] = dict(data=f['mod/sfc/alb_43_2130'][...], name='Surface albedo (lambertian)', units='N/A')
    ...
    fname_sfc = '%s/sfc_2130.pk' % fdir
    mod09 = sat_tmp(data)
    sfc0      = sfc_sat(sat_obj=mod09, fname=fname_sfc, extent=sat.extent, verbose=True, overwrite=overwrite)
    sfc_2d    = mca_sfc_2d(atm_obj=atm0, sfc_obj=sfc0, fname='%s/mca_sfc_2d_2130.bin' % fdir, overwrite=overwrite)
    # =================================================================================
    ...
    with h5py.File(f'{sat.fdir_pre_data}/pre-data.h5', 'r') as f_pre_data:
        ...
        data['rad_2d'] = dict(name='Gridded radiance'                , units='km'         , data=f_pre_data[f'mod/rad/rad_2130'][...])
        if solver.lower() == 'ipa':
    ...
    sca  = mca_sca(pha_obj=pha0, fname='%s/mca_sca_2130.bin' % fdir, overwrite=overwrite)
    # =================================================================================

    # mca_cld object
    # =================================================================================
    atm3d0  = mca_atm_3d(cld_obj=cld0, atm_obj=atm0, pha_obj=pha0, fname='%s/mca_atm_3d_2130.bin' % fdir)
...
def modis_2130_simulation_plot(sat, cfg_info, case_name_tag='default', fdir='tmp',
                               solver='3D', wvl=2130, plot=False):
    ...
    # save data
    # ==================================================================================================
    with h5py.File('data/%s/post-data_2130.h5' % case_name_tag, 'w') as f:
        f['wvl'] = wvl
        f['lon'] = lon_mod
        f['lat'] = lat_mod
        f['extent']         = extent
    ...
    if plot:
        ...
        plt.savefig(f'{sat.fdir_pre_data}/modis_2130_{case_name_tag}_{solver}.png', bbox_inches='tight')
        ...
        plt.subplots_adjust(hspace=0.4, wspace=0.4)
        plt.savefig(f'{sat.fdir_pre_data}/modis_2130_{case_name_tag}_{solver}_comparison.png', bbox_inches='tight', dpi=300)
        ...
        xy = np.vstack([rad_mod[logic], rad_rtm_3d[logic]])
        Z = gaussian_kde(xy)(xy)
        # Sort the points by density, so that the densest points are plotted last
        idx = Z.argsort()
        x_mod, y_rtm_3d, Z_den = rad_mod[logic][idx], rad_rtm_3d[logic][idx], Z[idx]
        ...
        ax3.set_xlabel('MODIS Measured Radiance\n($\mathrm{W\;m^{-2}\;\mu\;m^{-1}\;sr^{-1}}$)', fontsize=label_size)
        ax3.set_ylabel('Simulated %s Radiance\n($\mathrm{W\;m^{-2}\;\mu\;m^{-1}\;sr^{-1}}$)' %solver, fontsize=label_size)
        ...
        plt.subplots_adjust(hspace=0.4, wspace=0.4)
        plt.savefig(f'{sat.fdir_pre_data}/modis_2130_{case_name_tag}_{solver}_comparison_scatter.png', bbox_inches='tight', dpi=300)
        ...
        plt.subplots_adjust(hspace=0.4, wspace=0.4)
        plt.savefig(f'{sat.fdir_pre_data}/modis_2130_cloud_information.png', bbox_inches='tight')
        plt.clf()
        fig = plt.figure(figsize=(15, 5))
        ax1 = fig.add_axes([0.05, 0.05, 0.25, 0.9])
        ax2 = fig.add_axes([0.40, 0.05, 0.25, 0.9])
